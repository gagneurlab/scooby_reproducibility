...
def train(config):
    ...
    accelerator.load_state('/data/ceph/hdd/project/node_09/semi_supervised_multispecies/Downstream/single_cell/data/onek1k_bam_files/scooby_models/scooby_epoch_28_4000_scooby_onek1k_transform_emb_fixed')
    ...
    # Training loop
    for epoch in range(28, num_epochs):
        for i, [inputs, rc_augs, targets, cell_emb_idx] in tqdm.tqdm(enumerate(training_loader)):
            ...
            scheduler.step()
            if i % eval_every_n == 0:
                evaluate(accelerator, scooby, val_loader, mode='rna', stop_idx=0) 
                scooby.train()
